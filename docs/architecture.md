...
This repository uses a single canonical package, `synth_platform`, for one synthetic-data platform with multiple SSOT input workflows. The architecture separates **what the product does** (workflows/use cases/domain) from **how it talks to external systems** (infrastructure) and **how users invoke it** (interfaces).
