param(
  ...
  [string]$Model = $env:SP_PLATFORM_SLM_MODEL,
  [string]$GuardrailsConfigPath = $env:SP_NVIDIA_GUARDRAILS_CONFIG_PATH,
...
if (-not $Model) {
  throw "Set -Model or SP_PLATFORM_SLM_MODEL to the local/internal OpenAI-compatible model id."
}
...
if ($GuardrailsConfigPath) {
  $env:SP_NVIDIA_GUARDRAILS_CONFIG_PATH = $GuardrailsConfigPath
}
