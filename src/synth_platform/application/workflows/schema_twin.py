...
def prepare_schema_for_generation(
    ...
    *,
    ...
    for table in prepared.tables:
        table.row_count = max(1, int((table_row_counts or {}).get(table.name, rows)))
        if not (table.description or "").strip():
            table.description = _friendly_table_description(table.name)
        align_unique_int_ranges(prepared.columns.get(table.name, []), table.row_count)
    if locale:
...
def generate_from_schema(
    ...
    *,
    ...
    max_table_rows = max((max(1, int(table.row_count or 1)) for table in prepared.tables), default=max(1, row_count))
    config = PipelineConfig(
        generation_mode="schema_driven",
        seed=int(seed),
        preview_rows=max(1, min(int(preview_rows), max_table_rows)),
        full_rows=total_rows,
        preview_only=False,
        export_format=export_format,
        output_dir=Path(output_dir),
        write_reports=True,
        chunk_size=min(10_000, max(1_000, max_table_rows)),
        llm_text_enabled=bool(llm_text_enabled),
