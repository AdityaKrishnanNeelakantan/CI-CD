...
def is_docling_available() -> bool:
    """Return whether Docling can run in this local app environment.

    Importability is the supported local gate. Docling itself handles
    runtime model/artifact failures during conversion.
    """
    if importlib.util.find_spec("docling") is None:
        return False
    if os.getenv("SP_FORCE_DOCLING_AVAILABLE") == "1":
        return True
    artifacts_path = os.getenv("DOCLING_SERVE_ARTIFACTS_PATH")
    if artifacts_path and not Path(artifacts_path).exists():
        return False
    return True
...
def _build_docling_converter() -> Any:
    ...
    pipeline_options = PdfPipelineOptions(
        ...
        document_timeout=float(os.getenv("SP_DOCLING_DOCUMENT_TIMEOUT", "120")),
        do_ocr=_env_flag("SP_DOCLING_OCR", default=False),
        do_table_structure=_env_flag("SP_DOCLING_TABLE_STRUCTURE", default=False),
        force_backend_text=_env_flag("SP_DOCLING_FORCE_BACKEND_TEXT", default=True),
    )
