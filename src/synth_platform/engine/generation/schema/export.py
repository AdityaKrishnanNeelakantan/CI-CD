"""
Export utilities for MVP-generated tables.
...
def to_parquet(
    ...
    Example::
        ...
        tables = mvp.generate("A SaaS company with 1000 users.", seed=42)
        paths  = mvp.to_parquet(tables, "./data/")
        # ./data/users.parquet
...
def to_duckdb(
    ...
    *,
    ...
    Example::
        ...
        tables = mvp.generate("A fintech company with 2000 customers.", seed=42)
        conn   = mvp.to_duckdb(tables)
    ...
    Persistent file::
        ...
        conn = mvp.to_duckdb(tables, db_path="./analytics.duckdb")
    """
    try:
        import duckdb
    except ImportError as e:
        raise ImportError(
            "duckdb is required for DuckDB export.  Install with: pip install \"mvp[export]\""
        ) from e
...
def to_jsonl(
    ...
    Example::
        ...
        paths = mvp.to_jsonl(tables, "./data/")
    """
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
