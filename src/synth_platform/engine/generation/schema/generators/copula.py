...
try:
    ...
    SDV_AVAILABLE = False
    warnings.warn("SDV not installed. Install with: pip install 'mvp[advanced]'")
...
class CopulaGenerator:
    ...
    def fit(self, df: pd.DataFrame, metadata: Optional[Dict] = None) -> None:
        ...
        if not SDV_AVAILABLE:
            raise ImportError("SDV not installed. Install with: pip install 'mvp[advanced]'")

        from synth_platform.engine.generation.schema.datetime_utils import infer_datetime_format

        table_name = "table"
        self.metadata = Metadata()
        self.metadata.detect_table_from_dataframe(table_name=table_name, data=df)
        detected = self.metadata.to_dict().get("tables", {}).get(table_name, {}).get("columns", {})
        ...
        if metadata:
            for col, col_meta in metadata.items():
                if col not in df.columns:
                    continue
                update_kwargs = dict(col_meta)
                sdtype = update_kwargs.pop("sdtype", None)
                if sdtype:
                    if sdtype == "datetime" and "datetime_format" not in update_kwargs:
                        existing = detected.get(col, {})
                        update_kwargs["datetime_format"] = existing.get("datetime_format") or infer_datetime_format(
                            df[col]
                        )
                    self.metadata.update_column(col, table_name=table_name, sdtype=sdtype, **update_kwargs)

        self.metadata = Metadata.load_from_dict(self.metadata.to_dict())
