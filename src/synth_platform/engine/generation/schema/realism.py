...
from synth_platform.engine.inference.schema.domain_capsule import DomainCapsule
from synth_platform.engine.generation.schema.vocab_seeds import (
    CITIES_BY_COUNTRY,
    FIRST_NAMES,
...
class RealisticTextGenerator:
    ...
    def generate(
        ...
        def _has_non_fallback_capsule_vocab(key: str) -> bool:
            ...
            return any(
                getattr(provenance, "source_name", "") != "mvp-defaults"
    ...
    def _generate_latitude(self, *, size: int) -> np.ndarray:
        from synth_platform.engine.generation.schema.vocab_seeds import CITY_GEODATA
    ...
    def _generate_restaurant_name(self, *, size: int) -> np.ndarray:
        from synth_platform.engine.generation.schema.vocab_seeds import RESTAURANT_NAMES
    ...
    def _generate_menu_item(self, *, size: int, table_data: Optional[pd.DataFrame] = None) -> np.ndarray:
        from synth_platform.engine.generation.schema.vocab_seeds import MENU_ITEMS_BY_CATEGORY
    ...
    def _generate_comment_body(self, *, size: int) -> np.ndarray:
        from synth_platform.engine.generation.schema.vocab_seeds import COMMENT_BODIES
        pool = self._vocabulary("comment_body", COMMENT_BODIES)
        return np.array([self.rng.choice(pool) for _ in range(size)])

    def _generate_research_project_name(self, *, size: int) -> np.ndarray:
        from synth_platform.engine.generation.schema.vocab_seeds import RESEARCH_PROJECT_NAMES
