...
from synth_platform.engine.generation.schema.assets import AssetStore
...
from synth_platform.engine.generation.schema.generators_legacy import TextGenerator
from synth_platform.engine.generation.schema.noise import NoiseInjector
from synth_platform.engine.inference.schema.domain_priors import apply_domain_priors
...
from synth_platform.engine.generation.schema.smart_values import (
    ...
    generate_iban,
    generate_ifsc_code,
    generate_luhn_number,
    generate_routing_number,
    generate_ssn,
    generate_swift_bic,
)
from synth_platform.engine.validation.schema.validation import StreamingDataValidator
from synth_platform.engine.generation.schema.vocabulary import SemanticVocabularyGenerator
from synth_platform.engine.generation.schema.workflows import WorkflowEngine
...
class GenerationResult:
    ...
    text_generation: Dict[str, Any] = field(default_factory=dict)
...
class DataSimulator:
    ...
    def __init__(self, config: SchemaConfig,
        ...
        """
        from synth_platform.engine.validation.schema.validation import validate_schema
        ...
        self.llm_text_enabled = llm_text_enabled
        self.max_llm_rows = max_llm_rows
        ...
        # Apply semantic inference to fix column types / FK promotion.
        # Bare text columns otherwise default to lorem ``sentence`` generation.
        if apply_semantic_fixes:
            from synth_platform.engine.inference.schema.semantic import enrich_schema_semantics

            self.config = enrich_schema_semantics(self.config)

        # Set random seed if provided
        # Use SeedSequence instead of np.random.randint(0, 2**32 - 1).
        # On Windows/new NumPy builds that call can route through an int32 path
        # and fail with "ValueError: high is out of bounds for int32".
        seed = int(config.seed) if config.seed is not None else int(np.random.SeedSequence().generate_state(1)[0])
    ...
    def _banking_config(self) -> Any:
        """Return banking defaults from the schema, if configured."""
        return getattr(self.config, "banking", None)

    def _generate_llm_text_values(
        self,
        table_name: str,
        column: Column,
        size: int,
        ...
        """Generate one eligible text-heavy column via generic LLM + fallback layer."""
        params = column.distribution_params or {}
        domain = str(
            params.get("domain")
            or params.get("domain_hint")
            or getattr(self.config, "domain", None)
            or "generic"
        ).lower()
        locale = params.get("locale") or getattr(self, "locale", None) or "en_US"
        # Enable LLM when the global toggle and/or per-column flag is set.
        # Eligibility still gates quality; provider failures fall back to templates.
        column_llm_flag = bool(params.get("llm_enabled", params.get("llm_text", False)))
        config = TextGenerationConfig(
            llm_enabled=bool(self.llm_text_enabled or column_llm_flag),
            is_preview=self.is_preview,
            max_llm_rows=self.max_llm_rows,
            provider=str(params.get("llm_provider", self.llm_provider)),
            model=params.get("llm_model") or self.llm_model,
            locale=locale,
            domain=domain,
    ...
    def _generate_llm_banking_text_values(self, table_name: str, column: Column, size: int) -> np.ndarray:
        """Backward-compatible alias."""
        return self._generate_llm_text_values(table_name, column, size)

    def _generate_banking_values(self, column: Column, size: int) -> np.ndarray:
        """Generate synthetic banking primitives with valid format/check digits."""
        params = column.distribution_params or {}
        banking = self._banking_config()
        country = str(params.get("country") or getattr(banking, "country", "US")).upper()
        account_digits = int(params.get("digits") or getattr(banking, "account_number_digits", 12))
        synthetic_prefix = params.get("synthetic_prefix") or getattr(banking, "synthetic_prefix", None)
        col_type = column.type

        if col_type in {"bank_account", "account_number"}:
            return np.array([
                generate_account_number(account_digits, rng=self.rng, prefix=synthetic_prefix)
                for _ in range(size)
            ], dtype=object)

        if col_type == "routing_number":
            routing_format = params.get("format") or getattr(banking, "routing_number_format", "aba")
            if routing_format == "ifsc" or country == "IN":
                return np.array([generate_ifsc_code(self.rng) for _ in range(size)], dtype=object)
            return np.array([generate_routing_number(self.rng) for _ in range(size)], dtype=object)

        if col_type == "ssn":
            fmt = params.get("format") or getattr(banking, "ssn_format", "hyphenated")
            return np.array([generate_ssn(self.rng, hyphenated=(fmt != "plain")) for _ in range(size)], dtype=object)

        if col_type == "aadhaar":
            fmt = params.get("format") or getattr(banking, "aadhaar_format", "spaced")
            return np.array([generate_aadhaar(self.rng, spaced=(fmt != "plain")) for _ in range(size)], dtype=object)

        if col_type == "ifsc_code":
            return np.array([generate_ifsc_code(self.rng) for _ in range(size)], dtype=object)

        if col_type == "swift_bic":
            length = int(params.get("length", 11))
            return np.array([generate_swift_bic(self.rng, country=country, length=length) for _ in range(size)], dtype=object)

        if col_type == "iban":
            iban_country = params.get("iban_country") or getattr(banking, "iban_country", country if len(country) == 2 else "GB")
            return np.array([generate_iban(self.rng, country=iban_country) for _ in range(size)], dtype=object)

        if col_type == "credit_card":
            issuer = str(params.get("issuer", "visa")).lower()
            prefix = {"visa": "4", "mastercard": "5", "amex": "34"}.get(issuer, str(params.get("prefix", "4")))
            length = int(params.get("length", 15 if issuer == "amex" else 16))
            return np.array([generate_luhn_number(length=length, rng=self.rng, prefix=prefix) for _ in range(size)], dtype=object)

        raise ValueError(f"Unknown banking column type: {col_type}")
    ...
    def _generate_unique_text(self, text_type: str, size: int) -> np.ndarray:
        ...
        method_map = {
            ...
            "routing_number": lambda: generate_routing_number(self.rng),
            "account_number": lambda: generate_account_number(rng=self.rng),
            "bank_account": lambda: generate_account_number(rng=self.rng),
            "ssn": lambda: generate_ssn(self.rng),
            "aadhaar": lambda: generate_aadhaar(self.rng),
            "iban": lambda: generate_iban(self.rng),
            "swift_bic": lambda: generate_swift_bic(self.rng),
            "ifsc_code": lambda: generate_ifsc_code(self.rng),
            "credit_card": lambda: generate_luhn_number(rng=self.rng),
            "sentence":   self.text_gen.sentence,
            "word":       self.text_gen.word,
        }
        # Never use lorem sentences as unique primary keys — fall back to UUIDs.
        if text_type in {"sentence", "word"}:
            text_type = "uuid"
        gen_fn = method_map.get(text_type, method_map["uuid"])
        seen: set = set()
        results: list = []
    ...
    def generate_column(
        ...
        if column.type in ("int", "float") and self.locale != "en_US":
            from synth_platform.engine.inference.schema.domain_priors import apply_locale_priors
        ...
        if column.type in {"uuid", "email", "phone", "url", "address"}:
            shorthand = column.type
            proxy = column.model_copy(update={"type": "text", "distribution_params": {**params, "text_type": shorthand}})
            return self.generate_column(table_name, proxy, size, table_data)

        if column.type in {"currency"}:
            choices = params.get("choices")
            if not choices:
                banking = self._banking_config()
                choices = [getattr(banking, "currency", "USD")]
            return self.rng.choice(list(choices), size=size)

        if column.type in {"money", "decimal"}:
            proxy = column.model_copy(update={"type": "float", "distribution_params": params})
            return self.generate_column(table_name, proxy, size, table_data)

        if column.type in {
            "bank_account", "account_number", "routing_number", "ssn", "aadhaar",
            "iban", "swift_bic", "ifsc_code", "credit_card",
        }:
            return self._generate_banking_values(column, size)
        ...
        # INTEGER
            ...
            if column.unique:
                ...
                low, high = resolve_unique_int_bounds(params, total_needed_for_table)
        ...
        # TEXT
            ...
            explicit_text_type = "text_type" in params
            text_type = params.get("text_type", "sentence")
            llm_requested = bool(params.get("llm_text") or params.get("llm_enabled"))
            # Global Schema Twin toggle: route eligible text-heavy columns even when
            # the schema author did not set per-column llm_text flags.
            if self.llm_text_enabled and not llm_requested:
                from synth_platform.engine.generation.text.eligibility import assess_column_eligibility

                if assess_column_eligibility(column, table_name=table_name).eligible:
                    llm_requested = True
            if llm_requested:
                return self._generate_llm_text_values(table_name, column, size, table_data)
            if text_type in {
                "bank_account", "account_number", "routing_number", "ssn", "aadhaar",
                "iban", "swift_bic", "ifsc_code", "credit_card",
            }:
                proxy = column.model_copy(update={"type": text_type, "distribution_params": params})
                return self._generate_banking_values(proxy, size)
            ...
            # Default ``sentence`` without an explicit text_type is a schema gap —
            # prefer name-based realistic generation over lorem ipsum.
            if text_type == "sentence" and not explicit_text_type:
                return self.realistic_text.generate(
                    column_name=column.name,
                    table_name=table_name,
                    size=size,
                    semantic_type=text_strategy,  # None → name inference
                    table_data=table_data,
                )
    ...
    def _apply_time_density_curve(
        ...
        *,
        ...
        for window_start, count in zip(windows, counts):
            ...
            bucket_end = min(next_window, end + pd.Timedelta(1, unit="D"))
    ...
    def _apply_formula_columns(self, df: pd.DataFrame, table_name: str) -> pd.DataFrame:
        ...
        try:
            from synth_platform.engine.inference.schema.formulas import FormulaEngine
        except ImportError:
            return df
    ...
    def generate_with_reports(
        ...
        *,
        ...
        if self._text_generation_evidence:
            bundle.reports["text_generation"] = merge_evidence(self._text_generation_evidence)
        return GenerationResult(
            ...
            text_generation=bundle.reports.get("text_generation", {}),
        )
