"""
Time-series generation for MVP.
...
Quick start::
    ...
    # Story-driven
    ts = mvp.generate_timeseries(
        "Daily active users for a social app over 2 years, "
        "growing 15% monthly with weekly seasonality and a viral spike in June"
    )

    # Config-driven
    from synth_platform.engine.generation.schema.timeseries import TimeSeriesConfig, Trend, Seasonality, Anomaly
    ...
    ts = mvp.generate_timeseries(config=cfg)
"""
