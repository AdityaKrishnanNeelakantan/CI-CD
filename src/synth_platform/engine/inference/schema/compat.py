"""
Schema import utilities for MVP.
...
into MVP without rewriting them.

Supported input formats:
- Generic dict-based schemas (column name → type + constraints)
- SQLAlchemy ORM models (via ``mvp.schema_from_sqlalchemy``)
- YAML / JSON files (via ``mvp.load_recipe``)
...
Example::
    ...
    schema = mvp.from_dict_schema(schemas, row_count=2000)
...
def _col_from_dict(
    ...
    """Convert a single dict column definition to a MVP ``Column``."""
    raw_type = str(col_def.get("type", "string")).lower()

    # Primary keys → sequential unique int (name must be "id" for MVP's auto-sequence)
    ...
    if enum and isinstance(enum, list) and mvp_type in ("text", "int", "float"):
    ...
    if mvp_type == "categorical":
        ...
        params["choices"] = [str(c) for c in enum] if enum else ["Unknown"]
...
def from_dict_schema(
    ...
    """Convert a plain dict schema definition to a MVP ``SchemaConfig``.
...
def verify_integrity(
    ...
    MVP guarantees zero orphans during generation. Use this after manual
    ...
    Example::
        ...
        tables = mvp.generate("An ecommerce store with 5k orders", seed=42)
        report = mvp.verify_integrity(tables, schema)
