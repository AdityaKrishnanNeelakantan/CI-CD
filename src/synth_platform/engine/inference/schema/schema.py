"""
Pydantic models for MVP configuration.
...
class BankingConfig(BaseModel):
    """Locale-aware defaults for government/financial identifier formats.

    Used when schemas declare typed columns such as ``ssn``, ``iban``,
    ``routing_number``, or ``account_number``.  This is optional metadata —
    MVP works for any domain without it.
    """

    country: Literal["US", "IN", "GLOBAL"] = "US"
    currency: str = "USD"
    account_number_digits: int = Field(default=12, ge=6, le=18)
    routing_number_format: Literal["aba", "ifsc", "none"] = "aba"
    ssn_format: Literal["hyphenated", "plain"] = "hyphenated"
    aadhaar_format: Literal["spaced", "plain"] = "spaced"
    iban_country: str = "GB"
    ...
    def validate_currency(cls, value: str) -> str:
        ...
        if len(value) != 3 or not value.isalpha():
            raise ValueError("currency must be a 3-letter ISO-style code such as USD or INR")
        return value

    @field_validator("iban_country")
    @classmethod
    def validate_iban_country(cls, value: str) -> str:
        value = str(value or "GB").upper()
        if len(value) != 2 or not value.isalpha():
            raise ValueError("iban_country must be a 2-letter country code")
        return value


# Backward-compatible alias — ``BankingConfig`` remains the canonical schema field name.
LocaleConfig = BankingConfig
...
class Column(BaseModel):
    ...
    DISTRIBUTION_SHORTHAND_KEYS: ClassVar[tuple[str, ...]] = (
        "min",
        "max",
        "mean",
        "std",
    ...
    @classmethod
    def hoist_distribution_shorthand(cls, data: Any) -> Any:
        """Allow min/max and other generator params as top-level Column kwargs."""
        if not isinstance(data, dict):
            return data
        payload = dict(data)
        params = dict(payload.get("distribution_params") or {})
        for key in cls.DISTRIBUTION_SHORTHAND_KEYS:
            if key in payload and key not in params:
                params[key] = payload.pop(key)
        payload["distribution_params"] = params
        return payload
    ...
    def _normalize_distribution_params(
        ...
        if col_type == "categorical":
            ...
            if not choices:
                derived = normalized.pop("_derived_choices", None)
                if derived:
                    normalized["choices"] = [str(value) for value in derived]
                    normalized["_derived_from_profile"] = True
                elif normalized.get("allow_unknown_fallback"):
                    normalized["choices"] = ["Unknown"]
                    normalized.setdefault("probabilities", [1.0])
                    normalized["_categorical_fallback"] = True
                else:
                    normalized["_missing_categorical_choices"] = True
        ...
        if col_type in ["int", "float", "decimal", "money"] and "distribution" not in normalized:
            normalized["distribution"] = "normal"
            normalized["_distribution_is_default"] = True  # sentinel: not user-set

        if col_type in {"money", "currency"}:
            normalized.setdefault("decimals", 2)
            if col_type == "money":
                normalized.setdefault("min", 0.0)
                normalized.setdefault("max", 100000.0)
                normalized.setdefault("distribution", "uniform")

        if col_type in set(BANKING_COLUMN_TYPES):
    ...
    def validate_generation_ready(self) -> None:
        ...
        if self.type == "categorical":
            if params.get("_missing_categorical_choices") and not params.get("choices"):
                raise ValueError(
...
class SchemaConfig(BaseModel):
    ...
    banking: Optional[BankingConfig] = None
    ...
    def summary(self) -> str:
        ...
        if self.banking:
            lines.append(f"Banking: {self.banking.country} / {self.banking.currency}")
