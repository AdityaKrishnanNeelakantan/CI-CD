"""
First-class YAML schema format for MVP.
...
Quickstart::
    ...
    # 2. Edit mvp.yaml to match your domain
...
Python API::
    ...
    mvp.save_yaml_schema(schema, "mvp.yaml")
"""
...
from synth_platform.engine.inference.schema.compat import _TYPE_MAP
...
# ---------------------------------------------------------------------------
# Template — written by `mvp init` with no flags
# ---------------------------------------------------------------------------

MVP_YAML_TEMPLATE = """\
# yaml-language-server: $schema=https://raw.githubusercontent.com/rasinmuhammed/mvp/main/schema/mvp.schema.json
# mvp.yaml — synthetic data schema
# Run `mvp generate` to produce data from this file.
# Commit this file to git so your whole team can regenerate the same dataset.
#
# Docs: https://github.com/rasinmuhammed/mvp
...
# Optional banking defaults used by banking column types.
# banking:
#   country: US        # US | IN | GLOBAL
#   currency: USD      # ISO-style 3-letter code
#   account_number_digits: 12
#   routing_number_format: aba
#   ssn_format: hyphenated
#   aadhaar_format: spaced
...
_TEXT_SHORTHANDS = {"email", "phone", "url", "address", "name", "uuid"}
...
def _parse_column(col_name: str, col_def: Dict[str, Any]) -> Column:
    """Map a YAML column definition to a MVP Column."""
    raw_type = str(col_def.get("type", "text")).lower()
    if raw_type in _BANKING_TYPES:
        mvp_type = _BANKING_TYPE_ALIASES.get(raw_type, raw_type)
    else:
        mvp_type = _TYPE_MAP.get(raw_type, "text")

    # FK declared via relationships — column just needs type="foreign_key"
    if raw_type == "foreign_key" or mvp_type == "foreign_key":
        return Column(name=col_name, type="foreign_key", distribution_params={})

    # Categorical from choices
    choices = col_def.get("choices")
    if choices and isinstance(choices, list) and mvp_type in ("text", "int", "float"):
        mvp_type = "categorical"

    params: Dict[str, Any] = {}

    if mvp_type == "categorical":
        if not choices:
            params["allow_unknown_fallback"] = True
        params["choices"] = [str(c) for c in (choices or ["Unknown"])]
        probs = col_def.get("probabilities")
        if probs:
            params["probabilities"] = list(probs)

    elif mvp_type in ("int", "float"):
    ...
    elif mvp_type in ("decimal", "money"):
        for k in ("min", "max", "decimals"):
            if col_def.get(k) is not None:
                params[k] = col_def[k]
        if col_def.get("distribution"):
    ...
    elif mvp_type == "currency":
        params["choices"] = list(col_def.get("choices") or [col_def.get("code") or "USD"])

    elif mvp_type == "text":
        if col_def.get("text_type"):
            params["text_type"] = col_def["text_type"]
        elif raw_type in _TEXT_SHORTHANDS:
            params["text_type"] = raw_type

    elif mvp_type in _BANKING_TYPE_ALIASES.values() or mvp_type in _BANKING_TYPES:
        for k in (
            "digits", "format", "country", "prefix", "issuer", "length",
            "include_spaces", "iban_country", "synthetic_prefix",
        ):
            if col_def.get(k) is not None:
                params[k] = col_def[k]

    elif mvp_type in ("date", "datetime"):
        params["start"] = col_def.get("start", "2020-01-01")
        params["end"] = col_def.get("end", "2024-12-31")

    return Column(
        name=col_name,
        type=mvp_type,  # type: ignore[arg-type]
...
def load_yaml_schema(
    ...
    """Load a ``mvp.yaml`` schema file into a :class:`~mvp.schema.SchemaConfig`.
    ...
    Returns:
        :class:`~mvp.schema.SchemaConfig` ready for
        :func:`~mvp.generate_from_schema`.
    ...
    banking_raw = raw.get("banking") or raw.get("banking_config")
    banking = BankingConfig(**banking_raw) if isinstance(banking_raw, dict) else None
...
def save_yaml_schema(
    ...
    """Serialize a :class:`~mvp.schema.SchemaConfig` to a ``mvp.yaml`` file.
    ...
    Example::
        ...
        schema = mvp.parse("A SaaS company with 5k users")
        mvp.save_yaml_schema(schema, "mvp.yaml")
    """
    path = Path(path).expanduser()
    ...
    if getattr(schema, "banking", None) is not None:
        doc["banking"] = schema.banking.model_dump(exclude_none=True)
...
def _column_to_dict(col: Column) -> Dict[str, Any]:
    ...
    # Reverse-map MVP type to YAML type
    _REVERSE_TYPE = {
        "int": "int", "float": "float", "text": "text",
        "date": "date", "datetime": "datetime", "boolean": "boolean",
        "categorical": "categorical", "foreign_key": "foreign_key",
        "uuid": "uuid", "email": "email", "phone": "phone", "url": "url",
        "address": "address", "currency": "currency", "money": "money",
        "decimal": "decimal", "bank_account": "bank_account",
        "account_number": "account_number", "routing_number": "routing_number",
        "ssn": "ssn", "aadhaar": "aadhaar", "iban": "iban",
        "swift_bic": "swift_bic", "ifsc_code": "ifsc_code",
    ...
    for k in (
        "min", "max", "decimals", "distribution", "text_type", "start", "end",
        "digits", "format", "country", "prefix", "issuer", "length",
        "include_spaces", "iban_country", "synthetic_prefix",
    ):
        if k in p:
...
# ---------------------------------------------------------------------------
# JSON Schema (for IDE autocomplete + inline validation of mvp.yaml)
# ---------------------------------------------------------------------------

JSON_SCHEMA_URL = (
    "https://raw.githubusercontent.com/rasinmuhammed/mvp/"
    "main/schema/mvp.schema.json"
)


def json_schema() -> Dict[str, Any]:
    """Return the JSON Schema for ``mvp.yaml`` as a Python dict.
    ...
    Example::
        ...
        import json, mvp
        print(json.dumps(mvp.json_schema(), indent=2))
    """
    ...
    try:
        ...
        with resources.files("synth_platform.engine.inference.schema").joinpath("_schemas", "mvp.schema.json").open(
            "r", encoding="utf-8"
        ) as fh:
    ...
    repo_path = Path(__file__).parent / "_schemas" / "mvp.schema.json"
    if repo_path.exists():
        return _json.loads(repo_path.read_text(encoding="utf-8"))

    raise FileNotFoundError(
        "mvp.schema.json not found. Reinstall synth_platform or fetch it from "
        f"{JSON_SCHEMA_URL}"
    )


# Backward-compatible alias for old tests/docs.
MISATA_YAML_TEMPLATE = MVP_YAML_TEMPLATE
