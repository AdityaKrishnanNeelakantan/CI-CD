        ...
        if self._copula is not None:
            ...
            for column_name, encoder in self._encoders.items():
                if column_name in category_overrides and isinstance(encoder, CategoricalEncoder):
                    # Re-decode the SAME sampled numeric values through a
                    # rebalanced encoder instead of the original one - no
                    # re-fitting, see category_rebalancing.py's docstring
                    # for exactly what this does and doesn't preserve.
