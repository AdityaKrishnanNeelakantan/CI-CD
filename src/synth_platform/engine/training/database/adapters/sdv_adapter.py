...
def _load_sdv_classes() -> tuple[type[Any], type[Any]]:
    try:
        ...
        raise SynthesisError(
            ...
            "Use safe_gaussian_copula, or reinstall/upgrade sdv in the active virtualenv."
        ) from exc
    return Metadata, GaussianCopulaSynthesizer
...
class SDVSynthesizerAdapter(SynthesizerAdapter):
    ...
    def fit(
        ...
        Metadata, GaussianCopulaSynthesizer = _load_sdv_classes()
        metadata = Metadata()
    ...
    def load(cls, path: str | Path) -> SDVSynthesizerAdapter:
        ...
        try:
            ...
            _, GaussianCopulaSynthesizer = _load_sdv_classes()
