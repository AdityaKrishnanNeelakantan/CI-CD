...
class QualityReport:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Return a structured JSON-serialisable quality report."""
        return {
...
class DataQualityChecker:
    ...
    def check_typed_formats(
        self,
        df: pd.DataFrame,
        table_name: str,
        schema: Optional[Any] = None,
    ) -> None:
        """Validate typed-format columns using schema types when available."""
        column_types = schema_column_type_map(schema)
        for col in df.columns:
            column_type = column_types.get((table_name, col))
            invalid = invalid_typed_mask(col, df[col], column_type=column_type)
            if invalid is None:
                continue
            ...
            if invalid_count:
                self._add_issue(
                    ...
                    table_name,
                    col,
                    f"{invalid_count} invalid typed-format values in '{col}'",
                    {
                        "invalid_count": invalid_count,
                        "sample_values": df.loc[invalid, col].head(5).astype(str).tolist(),
                    },
                )

    def check_banking_primitives(
        self,
        df: pd.DataFrame,
        table_name: str,
        schema: Optional[Any] = None,
    ) -> None:
        """Backward-compatible alias for typed format checks."""
        self.check_typed_formats(df, table_name, schema=schema)

    def check_duplicate_density(
        self,
        tables: Dict[str, pd.DataFrame],
        schema: Optional[Any] = None,
    ) -> None:
        """Add generic advisory warnings for suspicious duplicate density.

        This is intentionally not a hard validation failure because real data can
        contain repeated names, categories, or descriptions.  The goal is to make
        unrealistic repetition visible before export.
        """
        if schema is None:
            return

        try:
            from synth_platform.engine.generation.schema.duplicate_guard import duplicate_quality_records

            records = duplicate_quality_records(tables, schema)
        except Exception:
            records = []
        ...
        for record in records:
            self._add_issue(
                "warning",
                record.get("category", "duplicate_density"),
                record.get("table", "unknown"),
                record.get("column"),
                record.get("message", "Duplicate density warning"),
                record.get("details", {}),
            )
    ...
    def check_time_series_properties(
        ...
        for col in numeric_cols:
            ...
            if len(series) > 10:
                ...
                # pandas 3.0 removed the fillna(method=...) keyword form.
    ...
    def check_all(
        ...
        self.check_duplicate_density(tables, schema)
        ...
        stats = {
            ...
            "format_checks": sum(1 for i in self.issues if i.category == "format"),
            "banking_checks": sum(1 for i in self.issues if i.category in {"format", "banking"}),
        }
