...
class FidelityReport:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Return a structured JSON-serialisable fidelity report."""
        return {
            "timestamp": self.timestamp,
            "overall_score": float(self.overall_score),
            "grade": self.grade,
            "heuristic": bool(self.heuristic),
            "results": [
                {
                    "name": result.name,
                    "test": result.test,
...
def _serialize_validation_report(report: Any) -> Dict[str, Any]:
    """Serialize MVP's validation report into the Oracle payload."""
    return {
...
class ReservoirTableSampler:
    ...
    PRIORITY_COLUMN = "__mvp_sample_priority__"
...
class FidelityChecker:
    ...
    def check_against_schema(self, tables: Dict[str, pd.DataFrame], schema_config: Any) -> FidelityReport:
        ...
        for table_name, df in tables.items():
            for column in schema_config.get_columns(table_name):
                ...
                if column.type in ("int", "float", "decimal", "money"):
                    ...
                    report.add_result(self._banking_result(table_name, column, series))
    ...
    def _typed_format_result(self, table_name: str, column: Any, series: pd.Series) -> FidelityResult:
        invalid = invalid_typed_mask(column.name, series, column_type=str(column.type))
        if invalid is None:
            return FidelityResult(name=f"{table_name}.{column.name}", test="typed format", score=90.0)
        checked = int(series.notna().sum())
        invalid_count = int(invalid.sum())
        score = 100.0 if checked == 0 else max(0.0, 100.0 * (1.0 - invalid_count / checked))
        return FidelityResult(
            name=f"{table_name}.{column.name}",
            test="typed format",
            score=round(score, 1),
            details={
                ...
                "valid_ratio": round(1.0 - invalid_count / max(checked, 1), 4),
            },
        )

    def _banking_result(self, table_name: str, column: Any, series: pd.Series) -> FidelityResult:
        """Backward-compatible alias."""
        return self._typed_format_result(table_name, column, series)
...
def build_data_card(tables: Dict[str, pd.DataFrame], schema_config: Any) -> DataCard:
    ...
    if getattr(schema_config, "banking", None):
        notes.append("Includes explicit identifier/locale generation configuration.")
    return DataCard(
        name=getattr(schema_config, "name", "MVP Dataset"),
...
def build_validation_report(
    ...
    *,
    ...
    """Build MVP validation report: a proof-oriented, shareable generation report.

    Combines hard validation signals with advisory realism, privacy, fidelity,
    locale, and reproducibility metadata in a machine-readable JSON shape.
    """
    ...
    if export_validation and export_validation.get("passed"):
        hard_passed = True
    elif export_validation and validation_scope == "full_export":
    ...
    from synth_platform.engine.validation.schema.validation_contract import build_validation_contract

    contract = build_validation_contract(
        hard_checks={
            "has_errors": getattr(validation, "has_errors", False) if not export_validation else False,
            "row_count_passed": row_fulfillment["passed"],
            "pk_passed": export_validation.get("pk_passed") if export_validation else None,
            "fk_passed": export_validation.get("fk_passed") if export_validation else None,
        },
        advisory_fidelity=advisory_fidelity,
        privacy=privacy,
        rule_evidence=rule_evidence,
        export_validation=export_validation,
        validation_scope=validation_scope,
        sampled_rows=int(sum(len(df) for df in tables.values())),
        exact_rows=int(sum(row_counts.values()) if row_counts else sum(len(df) for df in tables.values())),
    ...
    metrics_report = build_metrics_report(
        ...
        rule_evidence=rule_evidence,
        hard_validation_passed=bool(hard_passed),
        export_ready=bool(contract.export_ready),
        generation_mode=generation_mode,
    )
    ...
    report = {
        ...
        "summary": {
            ...
            "hard_validation_status": contract.hard_validation_status.value,
            "advisory_fidelity_status": contract.advisory_fidelity_status.value,
            "export_ready": bool(contract.export_ready),
            "validation_scope": validation_scope,
            ...
            "final_readiness": metrics_report.get("final_readiness", {}).get("status"),
            "metrics_passed": bool(metrics_report.get("final_readiness", {}).get("passed", False)),
        },
        "guarantees": {
            "validation": _serialize_validation_report(validation),
            "row_count_fulfillment": row_fulfillment,
            "constraints": constraint_summary,
            "export_validation": _json_safe(export_validation or {}),
        },
        "advisory": {
            ...
            "metrics": _json_safe(metrics_report),
        },
        "reproducibility": {
        ...
        "notes": [
            ...
            "Sampled advisory fidelity, not exact full-data validation." if sampled else "",
        ],
    }
    report["notes"] = [note for note in report["notes"] if note]
    return _json_safe(report)


# Backward-compatible alias (deprecated)
build_oracle_report = build_validation_report
