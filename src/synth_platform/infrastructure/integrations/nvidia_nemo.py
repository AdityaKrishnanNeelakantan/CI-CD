...
def run_guardrails_transcript_check(
    ...
    *,
    ...
    model = model or os.getenv("SP_NVIDIA_GUARDRAILS_MODEL") or os.getenv("SP_PLATFORM_SLM_MODEL")
    python_supported = sys.version_info < (3, 14)
    status = _status(
        "nemoguardrails",
        "nemoguardrails",
    ...
    try:
        ...
        config = _load_guardrails_config(config_path, model=model)
...
def _load_guardrails_config(config_path: str, *, model: str = ""):
    ...
    if path.is_dir():
        ...
        if not yaml_path.is_file():
            return RailsConfig.from_path(config_path)
        config_data = yaml.safe_load(yaml_path.read_text(encoding="utf-8")) or {}
        _apply_guardrails_model(config_data, model)
        colang_content = "\n\n".join(
            co_path.read_text(encoding="utf-8")
            for co_path in sorted(path.glob("*.co"))
        )
    ...
    _apply_guardrails_model(config_data, model)
    return RailsConfig.from_content(config=config_data)


def _apply_guardrails_model(config_data: dict[str, Any], model: str) -> None:
