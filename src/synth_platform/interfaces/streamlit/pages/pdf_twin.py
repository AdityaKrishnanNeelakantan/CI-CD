...
PDF_UI = ui_value("pdf", default={})

st.title(PDF_UI.get("title", "PDF Twin"))
platform_intro()
st.caption(
    PDF_UI.get(
        "caption",
...
defaults = {
    ...
    "pdf_llm_text": False,
    "pdf_intent": PDF_UI.get("default_intent", "Document testing"),
    "pdf_extraction_engine": PDF_UI.get("default_extraction_engine", "Automatic (native/OCR)"),
}
...
with st.container(border=True):
    ...
    step_guide(
        ...
        next_step=step.get("next", "Upload a PDF."),
    )
    intent_options = PDF_UI.get("intent_options", ["Document testing"])
...
# ---------------------------------------------------------------------------
with st.container(border=True):
    step = ui_step("pdf", "upload")
    step_header(2, step.get("title", "Upload"), st.session_state.pdf_source_path is not None)
    step_guide(
        what=step.get("what", "Provide the source PDF to twin."),
        next_step=step.get("next", "Discover document structure."),
    )
...
# ---------------------------------------------------------------------------
with st.container(border=True):
    step = ui_step("pdf", "discover")
    step_header(3, step.get("title", "Discover document"), st.session_state.pdf_profile is not None)
    step_guide(
        what=step.get("what", "Process the PDF so the platform can understand its structure."),
        next_step=step.get("next", "Build the twin template."),
    )
    extraction_options = PDF_UI.get("extraction_options", ["Advanced document extraction", "Automatic (native/OCR)"])
    ...
    st.session_state.pdf_extraction_engine = st.selectbox(
        ...
        help=(
            "Advanced extraction uses a document-layout and table-aware SDK and may initialize model artifacts on first use. "
            "Automatic uses the native-text path and falls back to OCR for scanned PDFs when OCR tooling is installed."
        ),
    )
    advanced_extraction_selected = st.session_state.pdf_extraction_engine in {
        "Advanced document extraction",
        "Docling",
    }
    if advanced_extraction_selected and not is_docling_available():
        st.warning(
            "Advanced document extraction is not installed in this environment. Install the PDF extras with "
    ...
    if st.session_state.pdf_profile is None:
        if st.button("Discover document", icon=":material/document_scanner:"):
            preferred_method = "docling" if advanced_extraction_selected else None
        ...
        c1.metric("Extraction method", _extraction_method_label(profile["extraction_method"]))
        ...
        with st.expander("Technical details - extraction", expanded=False):
            st.caption(
                f"{_extraction_method_label(profile['extraction_method'])} produced the profiling text and "
...
# ---------------------------------------------------------------------------
with st.container(border=True):
    step = ui_step("pdf", "template")
    step_header(4, step.get("title", "Build twin template"), st.session_state.pdf_template is not None)
    step_guide(
        what=step.get("what", "Capture structure and labels for the twin (sensitive values stay redacted)."),
        next_step=step.get("next", "Understand fields, then generate values."),
    )
...
with st.container(border=True):
    ...
    step = ui_step("pdf", "redacted")
    step_guide(what=step.get("what", "Download a redacted copy without generating a synthetic twin."))
...
# ---------------------------------------------------------------------------
with st.container(border=True):
    step = ui_step("pdf", "understand")
    step_header(5, step.get("title", "Understand fields"), st.session_state.pdf_binding_map is not None)
    step_guide(
        what=step.get(
            "what",
            "Map labels to semantic roles and entity paths so generation can stay realistic and consistent.",
        ),
        next_step=step.get("next", "Generate synthetic values."),
    )
    if st.session_state.pdf_binding_map is None:
        ...
        field_rows = [
            {
                ...
                "entity path": b.get("entity_path") or b.get("table_path") or "review",
                "confidence": b.get("confidence", b.get("semantic_confidence", "n/a")),
                "status": b.get("semantic_status", "approved"),
            }
        ...
        if field_rows:
            ...
            review_count = sum(1 for row in field_rows if str(row["status"]).upper() == "REVIEW_REQUIRED")
            if review_count:
                st.warning(f"{review_count} low-confidence binding(s) require review before production use.")
...
# ---------------------------------------------------------------------------
with st.container(border=True):
    step = ui_step("pdf", "generate")
    step_header(6, step.get("title", "Generate synthetic values"), st.session_state.pdf_values is not None)
    step_guide(
        what=step.get(
            "what",
            "Create AI-generated replacement values matched to each field's meaning. Shared entity/database values should be used when provided.",
        ),
        next_step=step.get("next", "Build and download the twin PDF."),
    )
    seed = st.number_input("Seed", value=SETTINGS.seed, key="pdf_seed")
    narrative_fields = list_pdf_narrative_bindings(st.session_state.pdf_binding_map)
    st.caption(
        "Names, addresses, dates, and notes are generated from the approved field meanings. "
    ...
    if st.button("Generate values", icon=":material/casino:"):
        with measure_generation() as run_stats:
            result = run_value_generation(
                ...
                generation_backend=SETTINGS.pdf_generation_backend,
            )
        if not result.is_success():
            show_user_error(
    ...
    if st.session_state.pdf_values is not None:
        ...
        g4.metric("Generation", "AI-assisted" if SETTINGS.pdf_generation_backend != "current" else "Standard")
...
# ---------------------------------------------------------------------------
with st.container(border=True):
    step = ui_step("pdf", "render_validate")
    step_header(7, step.get("title", "Build twin document and validate"), st.session_state.pdf_validation_report is not None)
    step_guide(
        what=step.get("what", "Render the synthetic PDF, then check field match and layout fit."),
        next_step=step.get("next", "Download the twin when validation looks good."),
    )
    if st.session_state.pdf_rendered_path is None:
        ...
        if st.session_state.pdf_validation_report is None:
            ...
            st.caption(
                ...
                "Synthetic document preserving useful structure with evidence-based privacy safeguards."
            )
