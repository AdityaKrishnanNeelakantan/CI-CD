...
def test_training_report_shape(temp_sqlite_db: Path, tmp_path: Path):
    ...
    assert any(event["phase"] == "fit_model" for event in progress_events)
    assert any(event["status"] == "complete" for event in progress_events)
    ...
    for table_report in report["tables"].values():
        ...
        assert table_report["training_trace"]["source_rows_read"] > 0
        assert table_report["training_trace"]["fit_rows"] > 0
        assert table_report["training_trace"]["total_duration_ms"] >= 0
        assert "fit_model" in table_report["training_trace"]["phase_durations_ms"]
