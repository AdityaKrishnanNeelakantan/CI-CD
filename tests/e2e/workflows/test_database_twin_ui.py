...
def test_full_database_twin_wizard_with_gender_ratio_tweak():
    ...
    inputs = [n for n in at.number_input if "gender" in (n.key or "")]
    male_input = next(n for n in inputs if n.key.endswith("_male"))
    female_input = next(n for n in inputs if n.key.endswith("_female"))
    male_input.set_value(40.0).run()
    female_input.set_value(60.0).run()
    assert not at.exception
...
def test_default_generation_without_override_stays_close_to_the_source_ratio():
    ...
    learned_gender = at.session_state["db_loaded_artifact"].get_model("customer").get_category_distribution("gender")
    assert male_ratio == pytest.approx(learned_gender["male"], abs=0.08)
...
def test_database_twin_can_be_retrained_after_exporting_a_model_artifact():
    """Database mode is a retrainable ML workflow, not a one-shot demo.

    Retraining keeps the approved SSOT understanding, starts a fresh immutable
    run, and clears generated/downloaded outputs that depended on the previous
    model snapshot.
    """
    at = AppTest.from_file(APP_PATH, default_timeout=180)
    at.run()
    ...
    first_run_dir = at.session_state["db_manifest"].run_dir
    assert at.session_state["db_contract"] is not None
    assert at.session_state["db_training_report"] is not None
    assert at.session_state["db_artifact_path"] is not None

    _click(at, "Retrain ML twin")

    assert at.session_state["db_contract"] is not None
    assert at.session_state["db_training_report"] is None
    assert at.session_state["db_artifact_path"] is None
    assert at.session_state["db_loaded_artifact"] is None
    assert at.session_state["db_manifest"].run_dir != first_run_dir
