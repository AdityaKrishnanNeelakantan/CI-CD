...
_HAS_REAL_PDF_FIXTURES = PDFS_DIR.is_dir() and any(PDFS_DIR.glob("*.pdf"))

pytestmark = [
    pytest.mark.e2e,
    pytest.mark.skipif(
        not _HAS_REAL_PDF_FIXTURES,
        reason="examples/fixtures/pdfs real-world PDF fixtures are not present in this checkout",
    ),
]
