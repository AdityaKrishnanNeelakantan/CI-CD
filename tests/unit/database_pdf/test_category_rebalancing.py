...
def test_rebalance_column_shifts_the_observed_ratio_from_4_3_to_exact_2_3():
    ...
    assert int((rebalanced_decoded == "male").sum()) == 4_000
    assert int((rebalanced_decoded == "female").sum()) == 6_000


def test_rebalance_column_uses_largest_remainder_for_integer_row_counts():
    original = CategoricalEncoder.from_frequencies({"a": 1, "b": 1, "c": 1})
    sampled_encoded = pd.Series([0.9, 0.2, 0.6, 0.4, 0.1])

    rebalanced_decoded = rebalance_column(sampled_encoded, original, {"a": 33.3, "b": 33.3, "c": 33.4})

    counts = rebalanced_decoded.value_counts().to_dict()
    assert counts == {"a": 2, "b": 1, "c": 2}
