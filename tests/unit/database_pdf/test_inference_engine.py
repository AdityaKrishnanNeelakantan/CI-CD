...
def test_country_column_is_category_not_free_text():
    candidate = infer_single_column(
        "country",
        ["US", "IN", "US", "GB", "US", "IN"] * 4,
    )
    assert candidate["semantic_type"] == "category"
    assert candidate["status"] == STATUS_PROPOSED
    assert "name_pattern=country" in candidate["evidence"]


def test_city_column_is_category_not_free_text():
    candidate = infer_single_column(
        "city",
        ["New York", "Phoenix", "New York", "Austin", "Phoenix", "Austin"] * 4,
    )
    assert candidate["semantic_type"] == "category"
    assert candidate["status"] == STATUS_PROPOSED
    assert "name_pattern=city" in candidate["evidence"]
...
def test_merchant_name_column_is_company_context_not_free_text():
    candidate = infer_single_column(
        "merchant_name",
        ["Northwind Market", "Contoso Fuel", "Northwind Market", "Fabrikam Pharmacy"] * 10,
    )
    assert candidate["semantic_type"] == "free_text"
    assert "name_pattern=company" in candidate["evidence"]
