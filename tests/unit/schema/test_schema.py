"""
Unit tests for schema validation.
"""

import pytest
from pydantic import ValidationError

from synth_platform.engine.inference.schema.schema import Column, NoiseConfig, RealismConfig, Table, Relationship, SchemaConfig, ScenarioEvent


class TestColumn:
    """Tests for Column model."""
    
    def test_basic_int_column(self):
        """Test creating a basic integer column."""
        col = Column(
            name="age",
            type="int",
            distribution_params={"distribution": "uniform", "min": 18, "max": 65}
        )
        assert col.name == "age"
        assert col.type == "int"
        assert col.distribution_params["min"] == 18
    
    def test_categorical_missing_choices_uses_safe_fallback(self):
        """Lenient LLM/import path can opt into Unknown fallback without console warnings."""
        col = Column(
            name="status",
            type="categorical",
            distribution_params={"allow_unknown_fallback": True},
        )
        assert col.distribution_params["choices"] == ["Unknown"]
        assert col.distribution_params["probabilities"] == [1.0]
        assert col.distribution_params.get("_categorical_fallback") is True

    def test_categorical_missing_choices_without_fallback_flags_review(self):
        col = Column(name="status", type="categorical", distribution_params={})
        assert col.distribution_params.get("_missing_categorical_choices") is True
        assert "choices" not in col.distribution_params or not col.distribution_params.get("choices")
    
    def test_date_requires_start_end(self):
        """Date columns should get sensible defaults when omitted."""
        col = Column(name="created_at", type="date", distribution_params={})
        assert "start" in col.distribution_params
        assert "end" in col.distribution_params
    
    def test_valid_date_column(self):
        """Test valid date column with start/end."""
        col = Column(
            name="created_at",
            type="date",
            distribution_params={"start": "2023-01-01", "end": "2024-12-31"}
        )
        assert col.distribution_params["start"] == "2023-01-01"
    
    def test_text_column(self):
        """Test text column with text_type."""
        col = Column(
            name="email",
            type="text",
            distribution_params={"text_type": "email"}
        )
        assert col.distribution_params["text_type"] == "email"

    def test_distribution_shorthand_hoisted_into_params(self):
        col = Column(name="record_id", type="int", unique=True, min=1, max=10_000_000)
        assert col.distribution_params["min"] == 1
        assert col.distribution_params["max"] == 10_000_000


class TestTable:
    """Tests for Table model."""
    
    def test_basic_table(self):
        """Test creating a basic table."""
        table = Table(name="users", row_count=1000)
        assert table.name == "users"
        assert table.row_count == 1000
        assert table.is_reference is False
    
    def test_reference_table_with_inline_data(self):
        """Test reference table with inline data."""
        table = Table(
            name="plans",
            is_reference=True,
            inline_data=[
                {"id": 1, "name": "Free", "price": 0.0},
                {"id": 2, "name": "Premium", "price": 9.99},
            ]
        )
        assert table.is_reference is True
        assert len(table.inline_data) == 2
        assert table.inline_data[0]["price"] == 0.0

    def test_table_accepts_workflow_configuration(self):
        """Tables can declare workflow presets explicitly."""
        table = Table(name="orders", row_count=100, workflow_preset="order", workflow_config={"strict": True})
        assert table.workflow_preset == "order"
        assert table.workflow_config["strict"] is True


class TestRelationship:
    """Tests for Relationship model."""
    
    def test_basic_relationship(self):
        """Test creating a relationship."""
        rel = Relationship(
            parent_table="users",
            child_table="orders",
            parent_key="id",
            child_key="user_id"
        )
        assert rel.parent_table == "users"
        assert rel.child_table == "orders"


class TestSchemaConfig:
    """Tests for complete schema configuration."""
    
    def test_minimal_schema(self):
        """Test minimal valid schema."""
        schema = SchemaConfig(
            name="Test",
            tables=[Table(name="users", row_count=100)],
            columns={
                "users": [
                    Column(name="id", type="int", distribution_params={"distribution": "uniform", "min": 1, "max": 100}),
                    Column(name="email", type="text", distribution_params={"text_type": "email"}),
                ]
            }
        )
        assert schema.name == "Test"
        assert len(schema.tables) == 1
    
    def test_schema_with_relationships(self):
        """Test schema with parent-child relationships."""
        schema = SchemaConfig(
            name="Test",
            tables=[
                Table(name="users", row_count=100),
                Table(name="orders", row_count=500),
            ],
            columns={
                "users": [
                    Column(name="id", type="int", distribution_params={"distribution": "uniform", "min": 1, "max": 100}),
                ],
                "orders": [
                    Column(name="id", type="int", distribution_params={"distribution": "uniform", "min": 1, "max": 500}),
                    Column(name="user_id", type="foreign_key", distribution_params={}),
                ],
            },
            relationships=[
                Relationship(parent_table="users", child_table="orders", parent_key="id", child_key="user_id")
            ]
        )
        assert len(schema.relationships) == 1
    
    def test_missing_table_columns_fails(self):
        """Schema should fail if table has no columns."""
        with pytest.raises(ValidationError):
            SchemaConfig(
                name="Test",
                tables=[Table(name="users", row_count=100)],
                columns={}  # No columns for users!
            )
    
    def test_invalid_relationship_table(self):
        """Schema should fail if relationship references non-existent table."""
        with pytest.raises(ValidationError):
            SchemaConfig(
                name="Test",
                tables=[Table(name="users", row_count=100)],
                columns={
                    "users": [Column(name="id", type="int", distribution_params={"distribution": "uniform"})]
                },
                relationships=[
                    Relationship(parent_table="users", child_table="orders", parent_key="id", child_key="user_id")
                ]
            )

    def test_schema_with_typed_noise_config(self):
        """Schema should accept typed noise configuration."""
        schema = SchemaConfig(
            name="Test",
            tables=[Table(name="users", row_count=10)],
            columns={
                "users": [
                    Column(name="id", type="int", distribution_params={"distribution": "uniform", "min": 1, "max": 10}),
                    Column(name="name", type="text", distribution_params={"text_type": "name"}),
                ]
            },
            noise_config=NoiseConfig(mode="analytics_safe", null_rate=0.1, typo_rate=0.2),
        )

        assert schema.noise_config is not None
        assert schema.noise_config.mode == "analytics_safe"

    def test_noise_config_validates_bounds(self):
        """Noise rates should be bounded between 0 and 1."""
        with pytest.raises(ValidationError):
            NoiseConfig(null_rate=1.5)

    def test_schema_with_realism_config(self):
        """Schema should accept typed realism configuration."""
        schema = SchemaConfig(
            name="Test",
            tables=[Table(name="users", row_count=10)],
            columns={
                "users": [
                    Column(name="id", type="int", distribution_params={"distribution": "uniform", "min": 1, "max": 10}),
                    Column(name="name", type="text", distribution_params={"text_type": "name"}),
                ]
            },
            realism=RealismConfig(
                row_planning="heuristic",
                coherence="standard",
                workflow_mode="preset",
                reports=["privacy", "fidelity"],
                text_mode="realistic_catalog",
            ),
        )

        assert schema.realism is not None
        assert schema.realism.coherence == "standard"
        assert schema.realism.reports == ["privacy", "fidelity"]

    def test_realism_config_validates_positive_overrides(self):
        """Realism row count overrides must stay positive."""
        with pytest.raises(ValidationError):
            RealismConfig(row_count_overrides={"orders": 0})
