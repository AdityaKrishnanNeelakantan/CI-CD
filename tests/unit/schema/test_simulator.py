...
from synth_platform.engine.inference.schema.schema import Column, NoiseConfig, OutcomeCurve, RealismConfig, Relationship, SchemaConfig, Table
...
class TestDataSimulator:
    ...
    def test_unique_int_exact_max_does_not_warn(self):
        import warnings

        schema = SchemaConfig(
            name="ExactRange",
            seed=42,
            tables=[Table(name="users", row_count=100_000)],
            columns={
                "users": [
                    Column(
                        name="user_id",
                        type="int",
                        unique=True,
                        distribution_params={"min": 1, "max": 100_000},
                    )
                ]
            },
        )
        with warnings.catch_warnings(record=True) as caught:
            ...
            data = self.generate_and_collect(DataSimulator(schema))
        assert data["users"]["user_id"].nunique() == 100_000
        assert not any("too small for unique column" in str(item.message) for item in caught)

    def test_unique_int_shorthand_max_supports_large_tables(self):
        import warnings

        schema = SchemaConfig(
            name="LargeIds",
            seed=7,
            tables=[Table(name="records", row_count=50_000)],
            columns={
                "records": [
                    Column(name="record_id", type="int", unique=True, min=1, max=50_000),
                ]
            },
        )
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            data = self.generate_and_collect(DataSimulator(schema))
        assert data["records"]["record_id"].nunique() == 50_000
        assert not any("too small for unique column" in str(item.message) for item in caught)
...
class TestEventCascade:
    ...
    def _churn_schema(self):
        from synth_platform.engine.inference.schema.schema import ScenarioEvent
...
class TestDomainPriors:
    """Domain priors applied from explicit schema domain settings."""

    def test_saas_mrr_is_lognormal_shaped(self):
        """SaaS domain: mrr column should be right-skewed after domain priors apply."""
        ...
        schema = SchemaConfig(
            ...
            domain="saas",
            seed=42,
            tables=[Table(name="subscriptions", row_count=500)],
            columns={
                "subscriptions": [
                    Column(
                        name="id",
        ...
        mrr = tables["subscriptions"]["mrr"].dropna()
        assert (mrr >= 0).all()
        assert mrr.mean() > mrr.median(), "SaaS MRR should be right-skewed"

    def test_get_column_prior_saas_mrr(self):
        from synth_platform.engine.inference.schema.domain_priors import get_column_prior
...
class TestVocabularySeeds:
    ...
    def test_first_names_pool_is_large(self):
        from synth_platform.engine.generation.schema.vocab_seeds import FIRST_NAMES
        assert len(FIRST_NAMES) >= 100

    def test_last_names_pool_is_large(self):
        from synth_platform.engine.generation.schema.vocab_seeds import LAST_NAMES
        assert len(LAST_NAMES) >= 80

    def test_product_by_category_all_populated(self):
        from synth_platform.engine.generation.schema.vocab_seeds import PRODUCT_BY_CATEGORY
    ...
    def test_conditional_product_sampling(self):
        from synth_platform.engine.generation.schema.vocab_seeds import PRODUCT_BY_CATEGORY, sample_conditional
        import numpy as np
