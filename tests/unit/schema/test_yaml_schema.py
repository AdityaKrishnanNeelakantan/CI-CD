"""
Tests for mvp.yaml_schema — load/save round-trip, constraint parsing, and
...
from synth_platform.engine.inference.schema.yaml_schema import load_yaml_schema, save_yaml_schema, MVP_YAML_TEMPLATE, json_schema
...
class TestSaveYamlSchema:
    ...
    def test_save_creates_file(self, tmp_path):
        from synth_platform.engine.inference.schema.schema import Column, Table
        s = SchemaConfig(
            name="save_test",
...
class TestMVPYamlTemplate:
    def test_template_is_valid_yaml(self):
        ...
        assert isinstance(MVP_YAML_TEMPLATE, str)
        assert len(MVP_YAML_TEMPLATE) > 100
        assert "tables:" in MVP_YAML_TEMPLATE

    def test_template_contains_key_sections(self):
        assert "relationships:" in MVP_YAML_TEMPLATE
        assert "constraints:" in MVP_YAML_TEMPLATE

    def test_json_schema_loader_returns_mvp_schema(self):
        schema = json_schema()
        assert schema["title"] == "MVP YAML Schema"
        assert schema["$id"].endswith("/schema/mvp.schema.json")
